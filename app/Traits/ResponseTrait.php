...
trait ResponseTrait
{
  ...
  public function response($key, $msg, $data = [], $anotherKey = [], $page = false)
  {
    ...
    # res data
    if ([] != $data && (in_array($key, ['success', 'needActive', 'exception', 'needApproval']))) {
  ...
  public function approvalReturn($data)
  {
    return $this->response('needApproval', __('auth.waiting_approvement'), $data);
  }
