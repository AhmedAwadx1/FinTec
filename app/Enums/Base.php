...
abstract class Base
{
    ...
    public static function __callStatic($method, $args)
    {
        ...
        if ($method == 'forRoutes') {
            return (new static())->forRoutes(...$args);
        }

        if ($method == 'getRemainingStatus') {
            return (new static())->getRemainingStatus(...$args);
        }
    ...
    protected function forRoutes($value)
    {
        return $this->reflection->getConstants()[$value];
    }


    protected function getRemainingStatus($excludedStatus)
    {
        $constants = $this->reflection->getConstants();
        return array_filter($constants, function ($status) use ($excludedStatus) {
            return !in_array($status, $excludedStatus);
        });
    }
}
